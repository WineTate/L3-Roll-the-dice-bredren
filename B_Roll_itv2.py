...
make_statement("chur brutha, welcome to Roll it 13: the game", "🤑")
...
while comp_score < game_goal and user_score < game_goal:
    ...
    # Roll dice for the user and note if they got a double
    ...
    if user_points < comp_points:
        print("You start cuz you did not win\n")

    # if the user and computer roll equal points, the users is player 1...
    elif user_points == comp_points:
        print("you and the computer roll the same, but you start first\n")
    ...
    # associate player points with either the user or the user
    ...
    round_feedback = f"The {winner} won. The {loser}'s points have been reset back to zero"

    # double user points if eligible
    if winner == "user" and double_user == "yes":
        user_points = user_points * 2

    # Outside rounds loop - Update score with round points, only add points to the score of the computer / user
    comp_score += comp_points
    user_score += user_points

    # Generate round results and add it to the game history list
    game_results = (f"Round {rounds_played}: User Points: {user_points} | "
                    f"Computer Points {comp_points} The {winner} Won This Round! "
...
if user_score > comp_score:
    ...
    make_statement("The computer won, ggs chat", "😭")
